...
SVO* SVO::sample() { // esto es para una escena de testing
	SVO* root = new SVO(4);
	root->children[0] = new SVO(2);
	root->children[0]->material.color = { 255, 0, 0 };//R,G,B
...
SVO* SVO::terrain(int size) {
	...
	Noise noise; // VIENE DE Terrain.h
	SVO* root = new SVO(size);
	for (int x = 0; x != size; ++x) { // for es por cada plano x y z
		for (int z = 0; z != size; ++z) {
			...
			for (int y = 0; y < yMax; ++y) {// entender loop
				root->set( //comienza esto desde la raiz del arbol
					x, y, z,
...
SVO* SVO::fromVox(const char* filename) {// funcion revisada anteriormente hay una funcion dentro de otra, la funcion dentro se define denuevo
...
void SVO::set(size_t x, size_t y, size_t z, glm::uvec3 rgb, bool water) {
	...
	size_t index = int(x / float(size) * 2) * 4 + int(y / float(size) * 2) * 2 + int(z / float(size) * 2); // recorrido del arbol
	assert(index >= 0 && index <= 7);
	if (!children[index]) children[index] = new SVO(size / 2);
	children[index]->set(x % (size / 2), y % (size / 2), z % (size / 2), rgb, water);// recursivo
}

unsigned int SVO::memoryusageSVDAG(std::vector<int32_t> svdag) {
	return sizeof(int32_t) * svdag.size();
}

void SVO::toSVDAG(std::vector<int32_t>& result, std::vector<Material>& materials) { // aqui comienza la conversion a svdag
	std::unordered_map<size_t, size_t> hashToIndex; //tabla hash a index
	std::unordered_map<Material, size_t, MaterialHasher> materialToIndex; // material () a index
...
void SVO::toSVDAGImpl(
	...
	if (!materialToIndex.contains(material)) { //condicion si hay material, agregar material 
		materialToIndex[material] = materials.size();
		materials.push_back(material);
	}
	//comienza manipulacion de bits
	auto matID = materialToIndex[material];
	//std::cout << "matID size" << sizeof(matID) << " size_t size" << sizeof(size_t)<< std::endl;

	assert( matID < 1<<24 );// checkpoint, si no se cumple se termina el programa

	int bitmask = 0;
	int bitmaskIndex = result.size();
	result.push_back(-1); // placeholder for bitmask
	for (int i = 0; i < 8; i++) {// recorre los hijos
		if (children[i] == nullptr) continue; // para saber si el SVO fue cargado antes de llamar a la funcion,
		result.push_back(-1); // placeholder for children index
		bitmask |= (1 << i); // pregunta
	}
	result[bitmaskIndex] = bitmask | (matID << 8); // fill bitmask

	int cnt = 0;// contador
	for (int i = 0; i < 8; i++) { // denuevo recorre los hijos pero ahora los registra en al tabla hash 
