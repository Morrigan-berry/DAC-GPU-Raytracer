...
ushort * optimizationk(std::vector<int32_t>& list,int listLength, int * nkvalues){
...
FTRep* createFT(std::vector<int32_t>& list, uint listLength){
	...
	rep->iteratorIndex = (uint *) malloc(sizeof(uint)*rep->nLevels);
	for(j=0;j<rep->nLevels;j++)
 			rep->iteratorIndex[j]= -1;
...
uint accessFT_Dac(FTRep* listRep, uint param, bitRankDac* br_d, uint* data, uint* rs) {
	...
	for (j = 0; j < listRep->nLevels; j++)
		listRep->iteratorIndex[j] = -1;
	j = 0;

	uint ini = param - 1;// pos 0

	listRep->iteratorIndex[0] = ini;
	...
	while ((!bitget(data, pos))) {

		rankini = rank_t(br_d, listRep->levelsIndex[j] + ini - 1, data, rs) - listRep->rankLevels[j];
		ini = ini - rankini;

		listRep->iteratorIndex[j + 1] = ini;
...
void saveFT(FTRep * rep, char * filename){
	...
	size_t r = fwrite(&(rep->listLength),sizeof(uint),1,flist);
	r = fwrite(&(rep->nLevels),sizeof(byte),1,flist);
	r = fwrite(&(rep->tamCode),sizeof(uint),1,flist);
	r = fwrite(&(rep->tamtablebase),sizeof(uint),1,flist);
	r = fwrite(rep->tablebase,sizeof(uint),rep->tamtablebase,flist);	
	r = fwrite(rep->base_bits,sizeof(ushort),rep->nLevels,flist);
	r = fwrite(rep->base,sizeof(uint),rep->nLevels,flist);
	r = fwrite(rep->levelsIndex,sizeof(uint),rep->nLevels+1,flist);
	r = fwrite(rep->iniLevel,sizeof(uint),rep->nLevels,flist);
	r = fwrite(rep->rankLevels,sizeof(uint),rep->nLevels,flist);

	r = fwrite(rep->levels,sizeof(uint),(rep->tamCode/W+1),flist);
...
FTRep* loadFT(char * filename){
	...
	size_t r = fread(&(rep->listLength),sizeof(uint),1,flist);
	r = fread(&(rep->nLevels),sizeof(byte),1,flist);
	r = fread(&(rep->tamCode),sizeof(uint),1,flist);
	
	r = fread(&(rep->tamtablebase),sizeof(uint),1,flist);
	rep->tablebase = (uint *) malloc(sizeof(uint)*rep->tamtablebase);
	r = fread(rep->tablebase,sizeof(uint),rep->tamtablebase,flist);	
	
	rep->base_bits = (ushort *) malloc(sizeof(ushort)*rep->nLevels);
	r = fread(rep->base_bits,sizeof(ushort),rep->nLevels,flist);
	
	
	rep->base = (uint *) malloc(sizeof(uint)*rep->nLevels);
	r = fread(rep->base,sizeof(uint),rep->nLevels,flist);
	
	
	rep->levelsIndex = (uint *) malloc(sizeof(uint)*(rep->nLevels+1));
	r = fread(rep->levelsIndex,sizeof(uint),rep->nLevels+1,flist);
	
	rep->iniLevel = (uint *) malloc(sizeof(uint)*rep->nLevels);
	r = fread(rep->iniLevel,sizeof(uint),rep->nLevels,flist);

	rep->rankLevels = (uint *) malloc(sizeof(uint)*(rep->nLevels));
	r = fread(rep->rankLevels,sizeof(uint),rep->nLevels,flist);
	
	rep->levels = (uint *) malloc(sizeof(uint)*(rep->tamCode/W+1));	
	r = fread(rep->levels,sizeof(uint),(rep->tamCode/W+1),flist);
...
uint memoryUsage(FTRep* rep) {
	...
	+ sizeof(ushort)*rep->nLevels// base bits
	+ sizeof(uint)*(rep->nLevels+1) //levels index
	+ sizeof(uint)*(rep->nLevels)//
	+ sizeof(uint)*(rep->nLevels)
	+ sizeof(uint)*(rep->nLevels)
	+ sizeof(uint)*(rep->tamCode/W+1) // levels
	+ spaceRequirementInBits(rep->bS)/8
	+ sizeof(struct sFTRep);
}

uint memoryUsageGPU(FTRep* rep) {
	return sizeof(uint) * rep->tamtablebase //table base buffer
		+ sizeof(uint) // puntero a n_levels (originalmente era char nLevels)
		+ sizeof(uint) * rep->nLevels // base bits (uint)
		+ sizeof(uint) * (rep->nLevels + 1)// levels index
		+ sizeof(uint) * (rep->nLevels)//iterator index
		+ sizeof(uint) * (rep->nLevels)//ini level
		+ sizeof(uint) * (rep->nLevels)// rank levels
		+ sizeof(uint) * (rep->tamCode / W + 1) // levels
		+ spaceRequirementInBits(rep->bS) / 8 // me quede con esto porque se utiliza toda la estructura
		+ sizeof(struct sFTRep);// la estructura completa
}
...
uint nextFT(FTRep * listRep){
	...
	listRep->iteratorIndex[j]++;
	ini = listRep->iteratorIndex[j];	

	pos=listRep->levelsIndex[j]+ini;
	
	listRep->iteratorIndex[j]=pos;
	...
	while((!bitget(listRep->bS->data,pos))) {
		if(listRep->iteratorIndex[j+1]==-1)
		{
			rankini = rank(listRep->bS, listRep->levelsIndex[j]+ini-1) - listRep->rankLevels[j];
			ini = ini-rankini;
			listRep->iteratorIndex[j+1] = ini;	
