...
uint rank_t(bitRankDac* brd, uint i, uint* data, uint* rs) {
    uint a;
    if (i + 1 == 0) return 0;
    ++i;
    uint resp = rs[i / brd->s];
    uint aux = (i / brd->s) * (brd->factor);
    for (a = aux; a < i / W; a++)
...
void copyBitStruct(bitRankW32Int* br1, bitRankW32Int* br2) {
    br1->data = br2->data;
    br1->factor = br2->factor;
    br1->integers = br2->integers;
    br1->n = br2->n;
    br1->owner = br2->owner;
    br1->Rs = br2->Rs;
    br1->s = br2->s;
