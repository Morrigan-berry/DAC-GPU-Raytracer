...
layout(std430, binding = 4) buffer levels { uint levelsDac[]; };
// output for length
layout(std430, binding = 5) buffer levelsIndex { uint levelsIndexDac[]; }; 
layout(std430, binding = 6) buffer baseBits { uint baseBitsDac[]; }; 
layout(std430, binding = 7) buffer iteratorIndex { uint iteratorIndexDac[]; }; 
layout(std430, binding = 8) buffer tablebase { uint tablebaseDac[]; }; 
layout(std430, binding = 9) buffer nlevels { uint nlevelsDac;};
layout(std430, binding = 10) buffer iniLevel { uint iniLevelDac[]; }; 
layout(std430, binding = 11) buffer rankLevels { uint rankLevelsDac[]; };
//bitrank pointers
layout(std430, binding = 12) buffer dataBitRank { uint dataBRDac[]; }; 
layout(std430, binding = 13) buffer rSbitRank { uint rSBRDac[]; }; 
layout(std430, binding = 14) buffer bitRank { 
    bitRankW32Int bitRankDac;
 }; 

//de ultimo es bitrank modificado
layout(std430, binding = 15) buffer popcount {uint popcountData[]; }; 
// ultimo es popcount para calc de rank
...
uint bitread (in uint p,in uint len){ 
     uint answ;
     uint pos = p/W;
     p %= W;
     answ = levelsDac[pos] >> p;
     if (len == W)
	  { if (p != 0) answ |= (levelsDac[pos+1]) << (W-p);
	  }
     else { if (p+len > W) answ |= (levelsDac[pos+1]) << (W-p);
            answ &= (1<<len)-1;
	  }
     return answ;
   }

uint t_popcount(in uint x) {// sera x  in o inout?
    uint resp = popcountData[(x >> 0) & 0xff] + popcountData[(x >> 8) & 0xff] + popcountData[(x >> 16) & 0xff] + popcountData[(x >> 24) & 0xff];
    return resp;
}

//entrada original uint rank_t(brd,in uint i,in uint* data, uint* rs)
uint rank(in uint i) {
    uint a;
    if (i + 1 == 0) return 0;
    ++i;
    uint resp = rSBRDac[i / bitRankDac.s];
    uint aux = (i / bitRankDac.s) * (bitRankDac.factor);
    for (a = aux; a < i / W; a++){
        uint aux = dataBRDac[a];
        resp += t_popcount(aux); 
    }
    resp += t_popcount(dataBRDac[i / W] & ((1 << (i & mask31)) - 1));
    return resp;
}

uint accessFT_Dac(in uint param) {//args originales FTRep* listRep, uint param, bitRankDac* br_d, uint* data, uint* rs
	uint mult = 0;
	uint j; //antes era register uint j
	uint partialSum = 0;
	...
	for (j = 0; j < nlevelsDac; j++)
		iteratorIndexDac[j] = -1;
	j = 0;

	uint ini = param - 1;// pos 0

	iteratorIndexDac[0] = ini;

	partialSum = 0;
	//level = listRep->levels;

	pos = levelsIndexDac[j] + ini;


	mult = 0;

	cont = iniLevelDac[j] + ini * baseBitsDac[j];


	readByte = bitread( cont, baseBitsDac[j]);
	if (nlevelsDac == 1) {
		return readByte;
	}

	while ( bitget(dataBRDac, pos) == 0) {

		rankini = rank(levelsIndexDac[j] + ini - 1) - rankLevelsDac[j];
		ini = ini - rankini;

		iteratorIndexDac[j + 1] = ini;
	...
	partialSum = partialSum + (readByte << mult) + tablebaseDac[j];
...
bool findNodeAt(in vec3 position, out bool filled, out AABB boxout, out Material mat) {
  ...
  AABB box = AABB(vec3(Epsilon), vec3(RootSize)); // initialize struct AABB to root box
  for (int i = 0; i < 32; ++i) {
    int childrenIndex =
    ...
    uint bitmask = accessFT_Dac(index + 1);
    ...
    if (((bitmask >> childrenIndex) & 1) == 1) {
      index = int(accessFT_Dac(index + 2 + bitCount(bitmask & ((1 << childrenIndex) - 1))));//obs      
      level += 1;

      //index = svdagData[index + 1 + bitCount(bitmask & ((1 << childrenIndex) - 1))];
      //level += 1;
